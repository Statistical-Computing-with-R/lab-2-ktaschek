...
The dimensions for the surveys.csv are 30463 rows by 15 columns.

**3. What are the data types of the variables in this dataset?**
```{r}
#| label: get-datatypes

# Referenced: https://stackoverflow.com/questions/21125222/determine-the-data-types-of-a-data-frames-columns

sapply(survey_csv, class)
```
...
# Referenced: https://chatgpt.com/share/6701f393-3234-8009-bc09-a0e39637ee14

ggplot(data = survey_csv,
       mapping = aes(x = species, y = weight)) +
  geom_jitter(mapping = aes(color = "orange3", alpha = 0.7), show.legend = FALSE) +
...
Based on the results of the ANOVA F-test, we found a statistically significant difference in average species weight according to species 13 with a value of 19916 and the p-value < 0.0001, which is less than the significance level of 0.05. Therefore we reject the null hypothesis that the population mean weight is the same among all species. 
